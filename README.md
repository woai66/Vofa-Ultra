...
- 会话级有界命令历史、完整格式恢复，以及有限次数或持续运行的可取消周期发送。
...
行尾。连续完全相同的命令合并计数；历史同时限制为最多 100 条和 256 KiB payload，超限从最旧项淘汰。历史、
输入草稿和任务状态不会写入工作区、`localStorage` 或诊断报告，可从终端发送栏单独清空。输入框在第一行按上键
浏览旧命令，在最后一行按下键返回；回到历史末尾时会恢复浏览前的完整草稿和格式。

周期发送支持 20 ms 到 24 小时的非实时整数间隔，以及 1 到 100,000 次或持续模式。启动时会编码并冻结 payload、
格式和行尾；之后编辑输入框不会改变运行任务。调度器只允许一个 `send()` 在途，上一次完成后才开始下一次间隔，
不会用 `setInterval` 追赶遗漏周期或制造突发队列。用户停止、断开或故障、自动恢复开始、切换数据源或工作区、
打开回放和运行时卸载都会取消后续发送；重连后不会自动恢复旧任务。队列满、非法 HEX 或其他发送失败会明确停止
任务，真实 TX 回执仍沿用终端与 `.vucap` 录制链路。
