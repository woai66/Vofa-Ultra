...
- “行”按所选 LF、CRLF 或 CR 聚合原始字节，允许行、行尾和 UTF-8 字符跨越多个读取块；完整后才显示。
...
RX 记录方式只改变终端展示，不改变 FireWater/JustFloat parser、处理、统计、自动应答或 VUCAP 原始记录边界。
...
显隐、处理图、姿态映射、自动应答规则和命名快捷命令。“保存”更新当前工作区，“另存为”创建新副本。切换时
如有未保存更改，应用会要求确认。
