...
“数据终端”的 TEXT/HEX 切换只改变显示方式，不改变接收到的字节。暂停终端只停止追加可见记录，暂停期间的
RX/TX 不会在恢复后补录；协议解析、波形处理、录制和诊断仍继续。终端最多保留 800 条记录，超出后从最旧项
开始淘汰。单条记录的 HEX 最多保留前 2048 字节，TEXT 最多保留前 2048 个解码字符，超出部分显示省略号。
...
“导出终端记录”始终导出 800 条上限内的完整当前保留记录，搜索和方向过滤不会缩小导出范围，因此仍不能替代
无损捕获；需要保留完整原始字节时使用 `.vucap` 录制。“清空终端”也作用于全部保留记录，但不会清除波形或
捕获文件。
...
“波形”提供 5、15、30、60 秒窗口、暂停、清空和测量：

1. 至少取得两个有效采样点后，点击直尺图标“开启波形测量”。
2. 选择基础或派生通道，再在图中点击或移动 A/B 游标。
3. 读取 `tA`、`tB`、`Δt`、`1/Δt`、`yA`、`yB` 和 `Δy`。
4. 再次点击直尺关闭。测量会冻结视口，但数据链路仍在运行。

清空波形、切换协议/来源/处理图/工作区/回放时间线会结束当前测量，避免游标引用旧数据。

## 发送命令

发送栏支持 TEXT、HEX 和无行尾/LF/CRLF。HEX 输入使用完整字节，例如 `01 03 00 00 00 02 C4 0B`；非法字符、
半个字节、空内容或展开后超过 64 KiB 会在发送前拒绝。

命令变量菜单可插入序号和时间。TEXT 示例：
...
载入不会展开变量、写串口、生成 TX 或加入命令历史。空模板只能与 LF/CRLF 行尾组合保存。每个工作区最多 32 条，
...
3. 核对帧预览、字节数和 CRC，再点击“填入发送框”。
4. 构帧器关闭后，用原“发送”按钮确认发送；也可再展开周期发送设置。

填入动作不会写串口或产生历史，只把发送格式切到 HEX、行尾切到“无行尾”并替换草稿。站号 `0` 仅用于写广播，
没有响应；读取广播会被拒绝。地址范围、标准数量上限和多值字节数在生成前严格校验，不会截断或回绕。构帧器只
负责请求编码，不解析设备响应；响应仍以终端原始字节显示。
...
1. 添加 `input`，选择一个基础通道。
2. 添加 `affine`、`clamp`、`ema`、`moving_average` 或 `math`，选择其上游节点并设置参数。
...
`.csv`；失败保留部分文件供诊断，不中断串口或原始录制。Raw Data 不产生数值通道。
...
工作区保存来源、协议、串口参数、显示/发送格式、行尾、时间窗、自动滚动、通道显隐、处理图、姿态映射、自动
应答规则和命名快捷命令。“保存”更新当前工作区，“另存为”创建新副本。切换时如有未保存更改，应用会要求确认。
...
“导出当前”生成严格版本化 JSON；“导入”校验格式和全部字段，同名项目会生成编号副本且不会自动应用。未来
版本的本地配置会触发只读保护，当前版本不会覆盖原数据。详见[兼容性与弃用政策](compatibility.md)。
...
工作区规则保存。暂停、清空或重置统计只影响对应视图，不会停止串口 worker。
