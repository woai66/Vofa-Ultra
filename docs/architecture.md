...
全部缓存并写绝对 ISO 时间。模式使用独立 UI 本地键，不进入 Store、工作区 v10 或捕获格式。
...
工作区 v10 保存数据源、协议、串口参数、终端收发格式、TX 行尾、RX 记录方式、接收行尾与文本编码、自动滚动、
...
v10。本地 v0-v9 数据都会迁移到持久化版本 10 并写回。历史 v2-v8 的处理图 parser 只接受旧七类节点，标为 v8
的新转换节点会被拒绝；v9 迁移必须使用当前处理图 parser，保留字节/数值转换节点。高于版本 10 的数据拒绝降级并
保持只读，旧客户端不会覆写未来
...
3. Playwright 覆盖模拟器端到端链路、TX 回显、Canvas 有效像素、波形框选挂起/持续采集/回到实时、确定性单次
   ...
   显式发送、字节/数值互转标准向量、处理图转换链、通道展示隔离与工作区 v10、
