...
帧；Raw Data 使用 `56 55` magic、版本 `01`、`u8` 通道数、`u32 LE` 样本序号和逐通道 `float32 LE` 载荷的
确定性二进制记录，但 Raw parser 仍不解释该私有模拟记录。三者最终都复用 `ingestBytes`、录制和统计入口。运行中
修改配置由 UI 和 Store 双层拒绝，避免一个连接代次内混合两套时间与通道语义。
