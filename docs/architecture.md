...
每个 worker 的 RX 计数从零开始。Rust 成功读取一个非空块后，为它分配当前 `sequence`、首字节 `streamOffset` 和
连接内单调时间，再更新后端累计字节数/事件数并优先投递录制队列。读取线程只为前端复制一次原始块并使用
`try_send` 投递到 4 MiB / 256 块双重有界的
显示队列；Base64 编码和 `serial://data` 发送由独立 publisher 完成。队列满、publisher 退出或发送失败时只丢弃
UI 显示块并累计原因，不反压串口读取，也不终止连接或 VUCAP 录制。

数据事件只携带热路径核对必需的块字节数、连接内单调微秒时间和递增后的后端计数，不为每个块重复序列化队列
诊断。前端按 generation 独立核对这些字段，只把边界可信且未重复、未乱序的事件交给协议解析器。序号或流偏移
向前跳跃会立即累计为 IPC 缺口，重复、乱序、迟到代次、字节数不符和单调时间倒退分别保留计数。

worker 终止时先关闭显示队列发送端并等待 publisher 排空，最长等待 1 秒；超时后取消剩余显示块，不能让断开无限
等待 UI 发布路径。正常收尾保证已排队数据事件先于终态，异常迟到事件仍由 generation 和终态边界拒绝。连接状态
和终态带上显示队列的当前值、容量、峰值、丢弃数、publisher 失败/超时及后端计数。前端据此核对最后一个
可见事件之后的尾部缺口，并把结果标为“本次完整”或“发现异常”。
诊断 JSON schema v3 包含该快照，但不包含原始 RX 内容。完整性范围从 Rust `read` 成功开始，到前端 Store 接受事件
为止；它不能证明串口芯片、USB 链路、驱动或操作系统在 `read` 之前没有丢数据，也不能替代真实设备的长稳与
高波特率验证。显示队列丢弃也不代表 VUCAP 丢失，录制完整性仍需由捕获状态和文件校验独立判断。
...
| RX 显示队列 | 4 MiB / 256 块 | 隔离 UI 发布开销；满载时保录制、丢显示并留下诊断 |
...
磁盘。连续 TX record 按顺序拼接后才是线上的 TX 字节流，record 边界不表示一次发送命令。原生 RX record 同时
保存连接代次、块序号、流偏移和连接内单调时间；reader 会拒绝同代次序号/偏移缺口、时间回退和代次倒退。新代次
重新建立基线。独立 writer 线程负责 VUCAP v3 格式编码、分块 SHA-256、定期进度事件、flush、同步和带标记计数的
完成 footer。状态事件同时公布队列当前占用、容量、字节/条目最高水位和结构化终止原因，能够区分两类容量耗尽、
writer 断开、panic 与收尾故障。

录制开始时冻结数据源、协议、串口参数、应用版本、构建 ID、Unix 起始时间和单调时钟基准；匹配到当前串口枚举项
时还保存设备类型、厂商/产品、成对 VID/PID 和 USB 序列号 SHA-256，不保存序列号原文。构建 ID 与设备摘要都是
诊断元数据，不是签名或匿名凭据。工作区、处理图和命令可能体积较大或含敏感内容，当前 v3 明确不保存。记录和
命名标记使用相对单调微秒时间，避免系统时钟跳变影响后续回放排序。Tauri 模拟器数据和标记通过前端 1 MiB 有界
FIFO 串行调用同一 recorder。CaptureWorker 冻结数据源并区分串口 TX、串口 RX 与模拟器入口：串口 RX 必须携带
流位置，模拟器 record 禁止携带流位置，模拟器 IPC 也不能写入串口捕获；reader 会按文件头重复验证这些约束。
原生串口标记也进入 Rust writer 队列。浏览器预览不模拟文件落盘。暂停终端或波形只影响视图缓存，不影响录制。
...
继续运行。主动停止、断开和工作区切换则串行等待 writer 收尾。格式 reader 明确支持 v1 / v2 / v3，逐条读取并
限制头部、单条记录和标记标签；v3 在交付每个条目前验证独立摘要，可区分未知版本、损坏、截断和正常完成；详见
...
record 和 marker，方向过滤不移除 marker，summary 追加标记计数；v3 原生 RX record 另输出 `rxStream` 对象。
CSV 和原始二进制不增加流元数据，并继续跳过标记以保持纯数据契约；
...
footer 统计、v3 分块摘要和时间戳单调性；缺少 footer 或尾部截断只开放已验证的完整记录前缀，摘要不匹配等
其他损坏进入 `error`。扫描同时
...
VUCAP v2 / v3 扫描另外按文件顺序收集最多 512 个命名标记，并通过一次性会话事件发送到前端；v1 会话得到空集合。
