...
选择哪种范围都写原始 ISO 绝对时间。模式使用独立 UI 本地键，不进入 Store、工作区 v11 或捕获格式。
...
保存原始输入、TEXT / HEX、校验模式、行尾、变量数、最近展开字节数和发送时间；只有发送队列接受命令后才写入。
连续条目只有在 payload、格式、校验模式和行尾均相同时才更新重复计数，不重复保存 payload。每次追加同时按
100 条和 256 KiB 原始输入字节裁剪，单条超过总预算时不进入历史。输入框浏览历史前保存 payload、格式、校验模式
和行尾草稿，回到末尾时整体恢复；终端输出仍使用浏览器原生文本
选择，不接管拖选手势。
...
工作区 v11 保存数据源、协议、串口参数、终端收发格式、命令校验模式、TX 行尾、RX 记录方式、接收行尾与文本编码、
自动滚动、
...
v10；v1-v10 最后补充 `commandChecksum: "none"` 归一为 v11。本地 v0-v10 数据都会迁移到持久化版本 11 并写回。
历史 v2-v8 的处理图 parser 只接受旧七类节点，标为 v8 的新转换节点会被拒绝；v9 迁移必须使用当前处理图 parser，
保留字节/数值转换节点。高于版本 11 的数据拒绝降级并
保持只读，旧客户端不会覆写未来
...
3. Playwright 覆盖模拟器端到端链路、TX 回显、Canvas 有效像素、实时通道监视统计/冻结/390 px 布局、波形框选
   ...
   实时 RX 自动应答、快捷命令持久化与
   显式发送、字节/数值互转标准向量、处理图转换链、通道展示隔离与工作区 v11、
