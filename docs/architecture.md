...
| 双游标测量与区间统计 | 单个选中通道、最多 2,000 点 | 固定扫描范围，不随通道总数扩张 |
...
选择哪种范围都写原始 ISO 绝对时间。模式使用独立 UI 本地键，不进入 Store、工作区 v10 或捕获格式。
...
区间统计对吸附后的 A/B 时间形成顺序无关的闭区间，在被测通道最多 2,000 个可见点内扫描有限样本，计算样本数、
...
3. Playwright 覆盖模拟器端到端链路、TX 回显、Canvas 有效像素、波形框选挂起/持续采集/回到实时、固定 Y 量程
