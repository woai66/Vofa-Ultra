...
流式导出、有界数据处理图、实时数值日志和帧一致 3D 姿态视图。架构目标不是追求模块数量，而是让串口生命周期、数据
所有权和故障语义足够清楚，使后续协议扩展和插件系统可以增量加入。
...
冻结姿态、归零参考、终端记录、命令历史、周期任务、自动应答启用态/队列/计数/冷却、统计、暂停状态、处理计数、
熔断错误和滤波窗口都属于运行数据，不会进入快照。
