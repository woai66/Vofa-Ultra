...
工作区 v6 保存数据源、协议、串口参数、终端收发格式、行尾、自动滚动、波形时间窗、基础/派生通道显隐偏好、
...
自动应答规则，v1-v4 补充空快捷命令，再将 v1-v5 规范化为 v6；本地 v0-v5 数据都会迁移到持久化版本 6 并写回。
v6 才允许顶层发送、自动应答和快捷命令使用 CR-only；历史 v1-v5 仍按原枚举拒绝该值。高于版本 6
的数据拒绝降级并保持只读，旧客户端不会覆写未来
...
1. Vitest 覆盖字节编解码、命令模板语法/端序/上限、快捷命令解析/排序/容量/迁移、跨 chunk 协议解析、
   处理图编译/预算/跨批滤波状态、姿态
...
2. React 组件测试覆盖主要空状态、工作区命名、恢复控制、变量光标插入与错误反馈、命令草稿导航、历史菜单、
   快捷命令管理与零隐式 TX、
   周期任务、波形跟随挂起/恢复/数据保留、波形测量暂停恢复/选点/清理、处理图建图/循环拒绝/依赖删除/熔断重试、
   姿态自动映射/冻结/归零/
   视角复位/渲染错误、自动应答规则编辑/运行锁定、数值记录和导出控制。
3. Playwright 覆盖模拟器端到端链路、TX 回显、Canvas 有效像素、波形框选挂起/持续采集/回到实时、波形双游标
   测量、工作区文件往返、录制入口权限、
   Raw / FireWater / JustFloat 回放滑杆拖动零 IPC、提交单次 seek、实际吸附位置反馈、播放中切速不换代、
   虚拟列表、命令变量 TEXT / HEX 展开与非法表达式
   拒绝、周期发送计数与停止、实时 RX 自动应答、快捷命令持久化与显式发送、处理图派生通道与工作区 v6、
   捕获导出入口权限、窄屏溢出、短窗口布局和同一 USB
