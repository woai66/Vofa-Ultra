...
更晚 TX 后才完成并追加。`<time>` 的 `dateTime` 始终保留原始 ISO 绝对时间，搜索仍只检查 payload，导出仍覆盖
全部缓存并写绝对 ISO 时间。模式使用独立 UI 本地键，不进入 Store、工作区 v9 或捕获格式。
...
工作区 v9 保存数据源、协议、串口参数、终端收发格式、TX 行尾、RX 记录方式、接收行尾与文本编码、自动滚动、波形时间窗、基础/派生通道显隐偏好、
...
补充 `terminalRxTextEncoding: "utf-8"`；v1-v8 再统一归一为 v9。本地 v0-v8 数据都会迁移到持久化版本 9 并写回。
历史 v2-v8 的处理图 parser 只接受旧七类节点，标为 v8 的新转换节点会被拒绝。高于版本 9 的数据拒绝降级并
保持只读，旧客户端不会覆写未来
...
3. Playwright 覆盖模拟器端到端链路、TX 回显、Canvas 有效像素、波形框选挂起/持续采集/回到实时、确定性单次
   ...
   显式发送、字节/数值互转标准向量、处理图转换链与工作区 v9、
