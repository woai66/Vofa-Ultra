...
协议解析健康度、流式导出、有界数据处理图、实时数值日志和帧一致 3D 姿态视图。架构目标不是追求模块数量，
而是让串口生命周期、数据所有权和故障语义足够清楚，使后续协议扩展和插件系统可以增量加入。
...
暂停状态、处理计数、熔断错误和滤波窗口都属于运行数据，不会进入快照。
...
6. 标签 workflow 在三平台全部成功后复验当前 run 的完整产物，加入版本 CHANGELOG、source/run 绑定记录、
   目标专属供应链文件与统一 `SHA256SUMS`；创建前后都将远端标签解引用到 commit，并要求与触发源码一致。
7. 聚合产物通过受保护 Environment 创建 draft；v0.x 和 SemVer 预发布版本自动标记为 prerelease，自动化不
   Publish，也不标记 Latest。
8. 正式发布前必须补充候选包安装/卸载冒烟，以及真实串口的长稳、拔插、流控和高波特率测试。

## 扩展原则

新协议先实现无副作用的增量解析器和夹具测试，再注册到 UI。工作区、记录回放和插件系统必须复用同一数据
平面，不允许绕过缓存上限。处理图只能作为可选的派生层继续扩展，不得成为基础收发、波形、录制或协议配置的
前置条件。
