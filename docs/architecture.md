...
有界数据处理图、实时数值日志和帧一致 3D 姿态视图。架构目标不是追求模块数量，而是让串口生命周期、数据
...
阻塞期间遗漏的 tick。任务与手动发送共用 store 级在途互斥，快速重复点击不会并行写入。
...
工作区 v3 保存数据源、协议、串口参数、终端收发格式、行尾、自动滚动、波形时间窗、基础/派生通道显隐偏好、
处理图和姿态映射。姿态映射严格校验输入模式、角度单位、坐标系、通道 ID、当前模式的重复通道和派生输出引用；
处理图删除输出时会同时清除对应姿态引用。连接状态、generation/revision、端口枚举、波形点、测量锚点、最新或
冻结姿态、归零参考、终端记录、命令历史、周期任务、统计、暂停状态、处理计数、熔断错误和滤波窗口都属于运行
数据，不会进入快照。
...
格式标识、schema 版本和字段集合；名称、枚举、串口参数、图拓扑及通道键都会重新校验并重建，未知字段不会
传播到运行状态。严格的外部 v1 文件先补充禁用空图，外部 v1/v2 再补充默认空姿态映射并规范化为 v3；本地
v0/v1/v2 数据都会迁移到持久化版本 3 并写回。高于版本 3 的数据拒绝降级并保持只读，旧客户端不会覆写未来
...
1. Vitest 覆盖字节编解码、命令模板语法/端序/上限、跨 chunk 协议解析、处理图编译/预算/跨批滤波状态、姿态
   ...
   发送无重入/无追赶、数值日志前端队列、捕获导出任务隔离和迟到结果过滤。
