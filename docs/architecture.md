...
连接时运行时冻结配置，定时器每次只生成一个样本并交给当前协议的 `encodeSimulatorSample`，不会补发页面挂起期间
遗漏的样本。停止会清理定时器；再次启动创建新的样本计数器并从 0 开始。FireWater 与 JustFloat 继续使用原协议
帧，Raw Data 按实际通道数输出 `ch1..ch16`，三者最终都复用 `ingestBytes`、录制和统计入口。运行中修改配置由
UI 和 Store 双层拒绝，避免一个连接代次内混合两套时间与通道语义。
...
选择哪种范围都写原始 ISO 绝对时间。模式使用独立 UI 本地键，不进入 Store、工作区 v12 或捕获格式。
...
工作区 v12 保存数据源、协议、串口参数、模拟器信号/通道数/采样率、终端收发格式、命令校验模式、TX 行尾、RX
记录方式、接收行尾与文本编码、
自动滚动、
...
`simulatorConfig` 归一为 v12。本地 v0-v11 数据都会迁移到持久化版本 12 并写回。
历史 v2-v8 的处理图 parser 只接受旧七类节点，标为 v8 的新转换节点会被拒绝；v9 迁移必须使用当前处理图 parser，
保留字节/数值转换节点。高于版本 12 的数据拒绝降级并
保持只读，旧客户端不会覆写未来
...
3. Playwright 覆盖模拟器 16 通道配置、确定性重启、运行锁定、TX 回显、Canvas 有效像素、实时通道监视
   ...
   实时 RX 自动应答、快捷命令持久化与
   显式发送、字节/数值互转标准向量、处理图转换链、通道展示隔离与工作区 v12、
