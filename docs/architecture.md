...
LF、CRLF 或 CR，允许分隔符、行内容和 UTF-8 字符跨越任意读取块；完整后才统一解码，并以该行首个读取块的
...
工作区 v7 保存数据源、协议、串口参数、终端收发格式、TX 行尾、RX 记录方式与接收行尾、自动滚动、波形时间窗、基础/派生通道显隐偏好、
...
仍按原枚举拒绝该值。v1-v6 再补充 `terminalRxRecordMode: "chunk"` 与 `terminalRxLineEnding: "lf"`，统一归一为
v7；本地 v0-v6 数据都会迁移到持久化版本 7 并写回。高于版本 7 的数据拒绝降级并保持只读，旧客户端不会覆写未来
...
3. Playwright 覆盖模拟器端到端链路、TX 回显、Canvas 有效像素、波形框选挂起/持续采集/回到实时、波形双游标
   ...
   拒绝、周期发送计数与停止、RX 文本行任意分包/超限/残行/生命周期边界、实时 RX 自动应答、快捷命令持久化与
   显式发送、字节/数值互转标准向量、处理图派生通道与工作区 v7、
