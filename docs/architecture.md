...
格式和行尾应用到草稿，变量直到用户显式手动发送或启动周期任务才展开。空模板仅在 LF/CRLF 能产生非空字节时
...
工作区 v5 保存数据源、协议、串口参数、终端收发格式、行尾、自动滚动、波形时间窗、基础/派生通道显隐偏好、
...
自动应答规则，v1-v4 补充空快捷命令并规范化为 v5；本地 v0-v4 数据都会迁移到持久化版本 5 并写回。高于版本 5
...
3. Playwright 覆盖模拟器端到端链路、TX 回显、Canvas 有效像素、波形双游标测量、工作区文件往返、录制入口权限、
   ...
   拒绝、周期发送计数与停止、实时 RX 自动应答、快捷命令持久化与显式发送、处理图派生通道与工作区 v5、
