...
选择哪种范围都写原始 ISO 绝对时间。模式使用独立 UI 本地键，不进入 Store、工作区 v14 或捕获格式。
...
同一读取块内的多个协议帧保留相同的原始接收时间戳。默认时基只保证坐标不倒退，同刻样本按 `frameSequence`
区分，避免对齐时覆盖前序帧。固定时基使用独立的实时/回放采样计数器，按 `index / chartSampleRateHz` 生成从
当前片段起点开始的相对坐标；基础与派生通道按 `frameIndex` 共用坐标，触发观察和频谱也使用所配置的采样时基。
原始记录、数值日志与姿态使用的接收时间不改写。清图、换流及回放定位会重置相应计数器；仅修改处理图不会重置
仍保留的基础波形时钟。固定时基不检测丢失帧，实验性附加扩展与固定时基互斥，避免混合无法对齐的时间轴。
...
工作区 v14 保存数据源、协议、串口参数、模拟器信号/通道数/采样率、终端收发格式、命令校验模式、TX 行尾与文本
编码、RX 记录方式、接收行尾与文本编码、
自动滚动、
波形时间窗、固定采样率时基、基础/派生通道显隐偏好、FireWater/JustFloat 基础通道展示配置、处理图、姿态映射、自动应答规则和
...
`simulatorConfig` 归一为 v12；v1-v12 再补充 `terminalTxTextEncoding: "utf-8"` 归一为 v13，v1-v13 再补充
`chartSampleRateHz: null` 归一为 v14。本地 v0-v13 数据都会迁移到持久化版本 14 并写回。
历史 v2-v8 的处理图 parser 只接受旧七类节点，标为 v8 的新转换节点会被拒绝；v9 迁移必须使用当前处理图 parser，
保留字节/数值转换节点。高于版本 14 的数据拒绝降级并
保持只读，旧客户端不会覆写未来
...
3. Playwright 覆盖模拟器 16 通道配置、确定性重启、运行锁定、TX 回显、Canvas 有效像素、实时通道监视
   ...
   实时 RX 自动应答、快捷命令持久化与
   显式发送、字节/数值互转标准向量、处理图转换链、通道展示隔离与工作区 v14、
