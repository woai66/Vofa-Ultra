...
- 有界 RX 自动应答，支持有序 TEXT/HEX 字节规则、跨 chunk/重叠/NUL 匹配、安全命令模板响应和工作区 v5。
...
- 工作区 JSON 与本地状态升级为 v6；v1-v5 保持原有行尾枚举并无损迁移，只有 v6 接受 CR-only。
- 实时与回放通道按批写入环形缓冲，每个受影响通道只物化一次 2000 点快照，降低高吞吐主线程开销。
- 兼容性清单升级为 schema v2，以结构化记录实验性扩展标识、包格式、schema/API 版本和未来版本拒绝行为。
- 工作区 JSON 与本地状态升级为 v5，严格读取 v1-v5、迁移本地 v0-v4，并将文件上限提高到 2 MiB。
